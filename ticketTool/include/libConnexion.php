...
function changePassword($bdd,$userId,$tempPassword){
    $newPassword = hash('sha256',$tempPassword);
    $requete = "UPDATE utilisateur SET SHA_PASS =".$newPassword." WHERE id=".$userId."";
    try {
        $bdd->query($requete);
        return True;
    } catch (Error) {
